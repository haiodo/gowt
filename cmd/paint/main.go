...
func main() {
	...
	canvas := swt.NewCanvasParentStyle(&shell.Composite, swt.SWTNONE)
	canvas.SetBackgroundColor_196(display.GetSystemColor(swt.SWTCOLOR_WHITE))
	canvas.AddPaintListener(painter(func(e *swt.PaintEvent) {
		gc := e.Gc
		gc.SetBackground(display.GetSystemColor(swt.SWTCOLOR_DARK_GREEN))
		gc.FillRectangle(20, 20, 120, 80)
		gc.SetForeground(display.GetSystemColor(swt.SWTCOLOR_RED))
		gc.SetLineWidth(3)
		gc.DrawLine(20, 130, 280, 130)
		gc.SetForeground(display.GetSystemColor(swt.SWTCOLOR_BLUE))
		gc.DrawOval(160, 20, 120, 80)
		gc.SetForeground(display.GetSystemColor(swt.SWTCOLOR_BLACK))
