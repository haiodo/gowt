		...
		func() {
			...
			for i := int32(0); i < int32(len(this.objects)); i++ {
				...
				if object != (nil) {
					...
					_, ok149 := resourceImplAsCursor(object)
					if ok149 {
						cursors++
					}
					...
					_, ok151 := resourceImplAsGC(object)
					if ok151 {
						gcs++
					}
					_, ok152 := resourceImplAsImage(object)
					if ok152 {
						images++
					}
					_, ok153 := resourceImplAsPath(object)
					if ok153 {
						paths++
					}
					_, ok154 := resourceImplAsPattern(object)
					if ok154 {
						patterns++
					}
					_, ok155 := resourceImplAsRegion(object)
					if ok155 {
						regions++
					}
					_, ok156 := resourceImplAsTextLayout(object)
					if ok156 {
						textLayouts++
					}
					_, ok157 := resourceImplAsTransform(object)
					if ok157 {
						transforms++
					}
				}
