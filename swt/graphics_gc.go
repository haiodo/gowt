...
func (this *GC) CheckGC(mask int32) *cocoa.NSAutoreleasePool {
	...
	if this.data.XorMode {
		cond258 = cocoa.OSKCGBlendModeDifference
	} else {
		cond258 = cocoa.OSKCGBlendModeNormal
	}
	cocoa.OSCGContextSetBlendMode(this.Handle.GraphicsPort(), cond258)
	...
	if (state & GCFOREGROUND) != 0 {
		...
		if pattern != (nil) {
			...
			cond259 := cocoa.NSColorColorWithDeviceRed(color[0], color[1], color[2], float64(float32(this.data.Alpha)/255))
			this.data.Fg = cond259
			var fg *cocoa.NSColor = cond259
			fg.Retain()
			fg.SetStroke()
		}
	}
	if (state & GCFOREGROUND_FILL) != 0 {
		...
		if pattern != (nil) {
			...
			cond260 := cocoa.NSColorColorWithDeviceRed(color[0], color[1], color[2], float64(float32(this.data.Alpha)/255))
			this.data.Fg = cond260
			var fg *cocoa.NSColor = cond260
			fg.Retain()
			fg.SetFill()
		}
	...
	if (state & GCBACKGROUND) != 0 {
		...
		if pattern != (nil) {
			...
			cond261 := cocoa.NSColorColorWithDeviceRed(color[0], color[1], color[2], float64(float32(this.data.Alpha)/255))
			this.data.Bg = cond261
			var bg *cocoa.NSColor = cond261
			bg.Retain()
			bg.SetFill()
		}
	...
	if (state & GCLINE_WIDTH) != 0 {
		...
		if this.data.LineWidth == 0 {
			...
			cond262 = this.data.LineWidth
		}
		path.SetLineWidth(float64(cond262))
		switch this.data.LineStyle {
		case SWTLINE_DOT, SWTLINE_DASH, SWTLINE_DASHDOT, SWTLINE_DASHDOTDOT:
			state |= GCLINE_STYLE
		}
	}
	if (state & GCLINE_STYLE) != 0 {
		...
		if dashes != (nil) {
			...
			for i := int32(0); i < int32(len(lengths)); i++ {
				if width == 0 || this.data.LineStyle == SWTLINE_CUSTOM {
...
func (this *GC) CopyArea(image *Image, x int32, y int32) {
	if this.Handle == (nil) {
	...
	if image.Type != SWTBITMAP || image.impl.IsDisposed() {
...
func (this *GC) CopyAreaImageXYSrcImage(image *Image, x int32, y int32, srcImage int64) {
	if srcImage == 0 {
		return
	}
	var rep *cocoa.NSBitmapImageRep = image.GetRepresentation0()
...
func (this *GC) CreateString(string_ string, flags int32, draw bool) *cocoa.NSAttributedString {
	...
	if (flags&SWTDRAW_MNEMONIC) != 0 || (flags&SWTDRAW_DELIMITER) == 0 {
		var i int32 = 0
		var j int32 = 0
		for i < int32(len(chars)) {
			...
			j++
			chars[t267] = cond265
			var c uint16 = cond265
			switch c {
			case '&':
				{
					if (flags & SWTDRAW_MNEMONIC) != 0 {
	...
	var nsPoint3 cocoa.NSPoint = cocoa.NSPoint{}
	{
		...
		for ; i < int32(len(this.types)); func() {
			...
			case int8(SWTPATH_MOVE_TO):
				t268 := j
				j++
				nsPoint.X = this.points[t268]
				t269 := j
				j++
				nsPoint.Y = this.points[t269]
				bezierPath.MoveToPoint(nsPoint)
				break
			case int8(SWTPATH_LINE_TO):
				t270 := j
				j++
				nsPoint.X = this.points[t270]
				t271 := j
				j++
				nsPoint.Y = this.points[t271]
				bezierPath.LineToPoint(nsPoint)
				break
			case int8(SWTPATH_CUBIC_TO):
				t272 := j
				j++
				nsPoint2.X = this.points[t272]
				t273 := j
				j++
				nsPoint2.Y = this.points[t273]
				t274 := j
				j++
				nsPoint3.X = this.points[t274]
				t275 := j
				j++
				nsPoint3.Y = this.points[t275]
				t276 := j
				j++
				nsPoint.X = this.points[t276]
				t277 := j
				j++
				nsPoint.Y = this.points[t277]
			...
			case int8(SWTPATH_QUAD_TO):
				...
				t278 := j
				j++
				nsPoint2.X = this.points[t278]
				t279 := j
				j++
				nsPoint2.Y = this.points[t279]
				t280 := j
				j++
				nsPoint.X = this.points[t280]
				t281 := j
				j++
				nsPoint.Y = this.points[t281]
...
func (this *GC) DrawImage(image *Image, x int32, y int32) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if image == (nil) {
...
func (this *GC) DrawImageImageSrcXSrcYSrcWidthSrcHeightDestXDestYDestWidthDestHeight(image *Image, srcX int32, srcY int32, srcWidth int32, srcHeight int32, destX int32, destY int32, destWidth int32, destHeight int32) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) DrawImageImageDestXDestYDestWidthDestHeight(image *Image, destX int32, destY int32, destWidth int32, destHeight int32) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) DrawImageSrcImageSrcXSrcYSrcWidthSrcHeightDestXDestYDestWidthDestHeightSimple(srcImage *Image, srcX int32, srcY int32, srcWidth int32, srcHeight int32, destX int32, destY int32, destWidth int32, destHeight int32, simple bool) {
...
func (this *GC) DrawPath(path *Path) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if path == (nil) {
...
func (this *GC) DrawRectangleRect(rect *Rectangle) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if rect == (nil) {
...
func (this *GC) DrawStringStringXYIsTransparent(string_ string, x int32, y int32, isTransparent bool) {
	var cond282 int32
	if isTransparent {
		cond282 = SWTDRAW_TRANSPARENT
	} else {
		cond282 = 0
	}
	this.DrawTextStringXYFlags(string_, x, y, cond282)
}

func (this *GC) DrawText(string_ string, x int32, y int32) {
	this.DrawTextStringXYFlags(string_, x, y, SWTDRAW_DELIMITER|SWTDRAW_TAB)
}

func (this *GC) DrawTextStringXYIsTransparent(string_ string, x int32, y int32, isTransparent bool) {
	var flags int32 = SWTDRAW_DELIMITER | SWTDRAW_TAB
	if isTransparent {
		flags |= SWTDRAW_TRANSPARENT
	}
...
func (this *GC) DrawTextStringXYFlags(string_ string, x int32, y int32, flags int32) {
	...
	if length == 1 && (flags&SWTDRAW_TRANSPARENT) != 0 {
...
func (this *GC) GetTextData(string_ string) *GC_GCTextData {
	...
	if this.data.Font == (nil) || this.data.Font.Handle == (nil) {
		...
		cond283 = this.data.Font.Handle.Id
	}
	var key *GC_GCTextData_Key = NewGC_GCTextData_Key(string_, this.data.Alpha, cond283, this.data.Foreground)
...
func (this *GC) DoDrawText(string_ string, x int32, y int32, flags int32) {
	...
	if (flags & SWTDRAW_TRANSPARENT) == 0 {
...
func (this *GC) Equals(object any) bool {
	...
	_, ok284 := resourceImplAsGC(object)
	if !(ok284) {
		return false
	}
...
func (this *GC) FillGradientRectangle(x int32, y int32, width int32, height int32, vertical bool) {
	...
	if fromRGB.Equals(toRGB) {
		...
		gradient.DrawInRect(rect, float64(cond285))
		gradient.Release()
	}
}
...
func (this *GC) FillPattern(path *cocoa.NSBezierPath, pattern *Pattern) {
	this.Handle.SaveGraphicsState()
	path.AddClip()
...
func (this *GC) FillPath(path *Path) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if path == (nil) {
...
func (this *GC) FillRectangleRect(rect *Rectangle) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if rect == (nil) {
...
func (this *GC) StrokePattern(path *cocoa.NSBezierPath, pattern *Pattern) {
	this.Handle.SaveGraphicsState()
...
func (this *GC) GetBackgroundPattern() *Pattern {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_WIDGET_DISPOSED)
	}
...
func (this *GC) GetClippingRegion(region *Region) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if region == (nil) {
	...
	if this.data.ClipPath != (nil) {
		...
		for i := int32(0); i < count; i++ {
			...
			case cocoa.OSNSMoveToBezierPathElement:
				...
				t286 := pointCount
				pointCount++
				pointArray[t286] = int32(pt.X)
				t287 := pointCount
				pointCount++
				pointArray[t287] = int32(pt.Y)
				break
			case cocoa.OSNSLineToBezierPathElement:
				cocoa.OSMemmoveOverload3(&pt, points, int64(cocoa.NSPointSizeof))
				t288 := pointCount
				pointCount++
				pointArray[t288] = int32(pt.X)
				t289 := pointCount
				pointCount++
				pointArray[t289] = int32(pt.Y)
				break
...
func (this *GC) GetTransform(transform *Transform) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if transform == (nil) {
...
func (this *GC) Init(drawable Drawable, data *GCData, context int64) {
	...
	var cond291 int32
	if data.FillRule == SWTFILL_WINDING {
		cond291 = cocoa.OSNSNonZeroWindingRule
	} else {
		cond291 = cocoa.OSNSEvenOddWindingRule
	}
	data.Path.SetWindingRule(int64(cond291))
	data.Path.Retain()
}
...
func (this *GC) InitCGContext(cgContext int64) {
	...
	if (state & GCLINE_WIDTH) != 0 {
		...
		if this.data.LineWidth == 0 {
			...
			cond292 = this.data.LineWidth
		}
		cocoa.OSCGContextSetLineWidth(cgContext, float64(cond292))
		switch this.data.LineStyle {
		case SWTLINE_DOT, SWTLINE_DASH, SWTLINE_DASHDOT, SWTLINE_DASHDOTDOT:
			state |= GCLINE_STYLE
		}
...
func (this *GC) SetAdvanced(advanced bool) {
	...
	if !advanced {
		this.SetAlpha(0xFF)
		this.SetAntialias(SWTDEFAULT)
		this.SetBackgroundPattern(nil)
		this.SetClippingOverload2(nil)
		this.SetForegroundPattern(nil)
		this.SetInterpolation(SWTDEFAULT)
		this.SetTextAntialias(SWTDEFAULT)
...
func (this *GC) SetBackground(color *Color) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if color == (nil) {
...
func (this *GC) SetBackgroundPattern(pattern *Pattern) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) SetClippingOverload1(path *Path) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) SetClippingOverload2(rect *Rectangle) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if rect == (nil) {
...
func (this *GC) SetClippingOverload3(region *Region) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	...
	var cond293 *cocoa.NSBezierPath
	if region != (nil) {
		cond293 = region.GetPath()
	} else {
		cond293 = nil
	}
	this.SetClippingOverload4(cond293)
}
...
func (this *GC) SetFillRule(rule int32) {
	...
	switch rule {
	case SWTFILL_WINDING, SWTFILL_EVEN_ODD:
		break
	default:
		SWTErrorFn(SWTERROR_INVALID_ARGUMENT)
	}
	...
	var cond294 int32
	if rule == SWTFILL_WINDING {
		cond294 = cocoa.OSNSNonZeroWindingRule
	} else {
		cond294 = cocoa.OSNSEvenOddWindingRule
	}
	this.data.Path.SetWindingRule(int64(cond294))
}

func (this *GC) SetFont(font *Font) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) SetForeground(color *Color) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if color == (nil) {
...
func (this *GC) SetForegroundPattern(pattern *Pattern) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if pattern != (nil) && pattern.impl.IsDisposed() {
...
func (this *GC) SetLineAttributes(attributes *LineAttributes) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
	if attributes == (nil) {
	...
	if lineStyle != this.data.LineStyle {
		mask |= GCLINE_STYLE
		switch lineStyle {
		case SWTLINE_SOLID, SWTLINE_DASH, SWTLINE_DOT, SWTLINE_DASHDOT, SWTLINE_DASHDOTDOT:
			break
		case SWTLINE_CUSTOM:
			if attributes.Dash == (nil) {
				lineStyle = SWTLINE_SOLID
			}
			break
		default:
			SWTErrorFn(SWTERROR_INVALID_ARGUMENT)
	...
	if join != this.data.LineJoin {
		mask |= GCLINE_JOIN
		switch join {
		case SWTJOIN_MITER, SWTJOIN_ROUND, SWTJOIN_BEVEL:
			break
		default:
			SWTErrorFn(SWTERROR_INVALID_ARGUMENT)
		}
	...
	if cap != this.data.LineCap {
		mask |= GCLINE_CAP
		switch cap {
		case SWTCAP_FLAT, SWTCAP_ROUND, SWTCAP_SQUARE:
			break
		default:
			SWTErrorFn(SWTERROR_INVALID_ARGUMENT)
		}
...
func (this *GC) SetLineCap(cap int32) {
	...
	switch cap {
	case SWTCAP_ROUND, SWTCAP_FLAT, SWTCAP_SQUARE:
		break
	default:
		SWTErrorFn(SWTERROR_INVALID_ARGUMENT)
	}
...
func (this *GC) SetLineDash(dashes []int32) {
	...
	var lineDashes []float32 = this.data.LineDashes
	if dashes != (nil) && int32(len(dashes)) > 0 {
		var changed bool = this.data.LineStyle != SWTLINE_CUSTOM || lineDashes == (nil) || int32(len(lineDashes)) != int32(len(dashes))
		for i := int32(0); i < int32(len(dashes)); i++ {
		...
		this.data.LineStyle = SWTLINE_CUSTOM
	} else {
		if this.data.LineStyle == SWTLINE_SOLID && (lineDashes == (nil) || int32(len(lineDashes)) == 0) {
			return
		}
		this.data.LineDashes = nil
		this.data.LineStyle = SWTLINE_SOLID
	}
...
func (this *GC) SetPatternPhase(pattern *Pattern) {
...
func (this *GC) SetTransform(transform *Transform) {
	if this.Handle == (nil) {
		SWTErrorFn(SWTERROR_GRAPHIC_DISPOSED)
	}
...
func (this *GC) TextExtent(string_ string) *Point {
	return this.TextExtentStringFlags(string_, SWTDRAW_DELIMITER|SWTDRAW_TAB)
}
...
func (this *GC) TextExtentStringFlags(string_ string, flags int32) *Point {
	...
	var attribStr *cocoa.NSAttributedString = this.CreateString(cond295, flags, false)
	this.data.TextStorage.SetAttributedString(attribStr)
	attribStr.Release()
	...
	if length == 0 {
		...
		cond296 = int32(math.Ceil(float64(rect.Width)))
	}
	return NewPoint(cond296, int32(math.Ceil(float64(rect.Height))))
}
...
func GCCheckStyle(style int32) int32 {
	if (style & SWTLEFT_TO_RIGHT) != 0 {
		style &= ^SWTRIGHT_TO_LEFT
	}
	return style & (SWTLEFT_TO_RIGHT | SWTRIGHT_TO_LEFT)
}

func GCCocoa_new(drawable Drawable, data *GCData) *GC {
	var gc *GC = newGC()
