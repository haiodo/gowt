...
func NewTextLayout(device *Device) *TextLayout {
	this := &TextLayout{}
	this.impl = this
...
func (this *TextLayout) initTextLayout(device *Device) {
	...
	this.alignment = SWTLEFT
	this.orientation = SWTLEFT_TO_RIGHT
	this.text = ""
...
func (this *TextLayout) ComputeRuns() {
	...
	if (this.orientation & SWTRIGHT_TO_LEFT) != 0 {
	...
	var cond426 int32
	if this.wrapWidth != -1 {
		cond426 = cocoa.OSNSLineBreakByWordWrapping
	} else {
		cond426 = cocoa.OSNSLineBreakByClipping
	}
	paragraph.SetLineBreakMode(int64(cond426))
...
func (this *TextLayout) Draw(gc *GC, x int32, y int32) {
	this.DrawGcXYSelectionStartSelectionEndSelectionForegroundSelectionBackground(gc, x, y, -1, -1, nil, nil)
}

func (this *TextLayout) DrawGcXYSelectionStartSelectionEndSelectionForegroundSelectionBackground(gc *GC, x int32, y int32, selectionStart int32, selectionEnd int32, selectionForeground *Color, selectionBackground *Color) {
...
func (this *TextLayout) DrawGcXYSelectionStartSelectionEndSelectionForegroundSelectionBackgroundFlags(gc *GC, x int32, y int32, selectionStart int32, selectionEnd int32, selectionForeground *Color, selectionBackground *Color, flags int32) {
	this.CheckLayout()
	if gc == (nil) {
	...
	if hasSelection || ((flags&SWTLAST_LINE_SELECTION) != 0 && (flags&(SWTFULL_SELECTION|SWTDELIMITER_SELECTION)) != 0) {
		if selectionBackground == (nil) {
			selectionBackground = this.device.impl.GetSystemColor(SWTCOLOR_LIST_SELECTION)
		}
		...
		if hasSelection {
			...
			var pArray int64 = this.layoutManager.RectArrayForCharacterRange(range_, range_, this.textContainer, rectCount)
			{
				...
				for ; int64(k) < rectCount[0]; func() {
					...
					if (flags&(SWTFULL_SELECTION|SWTDELIMITER_SELECTION)) != 0 && ((flags & SWTLAST_LINE_SELECTION) != 0) {
	...
	if numberOfGlyphs > 0 {
		...
		if !defaultFg {
			for i := int32(0); i < this.stylesCount-1; i++ {
				...
				if run.style != (nil) && run.style.Underline && run.style.UnderlineStyle == SWTUNDERLINE_LINK {
					continue
				}
				if length != 0 {
					range_.Location = int64(this.TranslateOffset(run.start))
		...
		for j := int32(0); j < this.stylesCount; j++ {
			...
			var drawBorder bool = style.BorderStyle != SWTNONE
						...
						if range_.Length > 0 {
							...
							}
							{
								...
								for ; int64(k) < rectCount[0]; func() {
									...
									var cond427 int32
									if lengths != (nil) {
										cond427 = int32(len(lengths))
									} else {
										cond427 = 0
									}
									path.SetLineDash(lengths, int64(cond427), float64(0))
									path.AppendBezierPathWithRect(rect)
									path.Stroke()
								}
							}
...
func (this *TextLayout) GetLevel(offset int32) int32 {
	...
	if !(0 <= offset && offset <= length) {
		SWTErrorFn(SWTERROR_INVALID_RANGE)
	}
...
func (this *TextLayout) _getOffset(offset int32, movement int32, forward bool) int32 {
	...
	if (movement & SWTMOVEMENT_CHAR) != 0 {
		return offset + step
	}
	switch movement {
	case SWTMOVEMENT_CLUSTER:
		offset += step
		if 0 <= offset && offset < length {
...
func (this *TextLayout) GetOffset(point *Point, trailing []int32) int32 {
	this.CheckLayout()
	if point == (nil) {
		SWTErrorFn(SWTERROR_NULL_ARGUMENT)
	}
...
func (this *TextLayout) GetRanges() []int32 {
	...
	for i := int32(0); i < this.stylesCount-1; i++ {
		if this.styles[i].style != (nil) {
			t428 := count
			count++
			result[t428] = this.styles[i].start
			t429 := count
			count++
			result[t429] = this.styles[i+1].start - 1
		}
	}
...
func (this *TextLayout) GetSegmentsText() string {
	...
	if this.orientation == SWTRIGHT_TO_LEFT {
	...
	for charCount < length {
		if segmentCount < nSegments && charCount == this.segments[segmentCount] {
			...
			t430 := segmentCount
			segmentCount++
			newChars[charCount+t430] = separator
		} else {
			t431 := charCount
			charCount++
			newChars[charCount+segmentCount] = oldChars[t431]
		}
	}
	for segmentCount < nSegments {
		...
		t432 := segmentCount
		segmentCount++
		newChars[charCount+t432] = separator
	}
...
func (this *TextLayout) GetStyles() []*TextStyle {
	...
	for i := int32(0); i < this.stylesCount; i++ {
		if this.styles[i].style != (nil) {
			t433 := count
			count++
			result[t433] = this.styles[i].style
		}
	}
	if count != int32(len(result)) {
...
func (this *TextLayout) IsUnderlineSupported(style *TextStyle) bool {
	if style != (nil) && style.Underline {
		var uStyle int32 = style.UnderlineStyle
		return uStyle == SWTUNDERLINE_SINGLE || uStyle == SWTUNDERLINE_DOUBLE || uStyle == SWTUNDERLINE_LINK || uStyle == TextLayoutUNDERLINE_THICK
	}
	return false
}
...
func (this *TextLayout) SetAlignment(alignment int32) {
	this.CheckLayout()
	var mask int32 = SWTLEFT | SWTCENTER | SWTRIGHT
	alignment &= mask
	if alignment == 0 {
		return
	}
	if (alignment & SWTLEFT) != 0 {
		alignment = SWTLEFT
	}
...
func (this *TextLayout) SetFixedLineMetrics(metrics *FontMetrics) {
	if metrics == (nil) {
...
func (this *TextLayout) SetFont(font *Font) {
	this.CheckLayout()
...
func (this *TextLayout) SetOrientation(orientation int32) {
	...
	if (orientation & SWTLEFT_TO_RIGHT) != 0 {
		orientation = SWTLEFT_TO_RIGHT
	}
...
func (this *TextLayout) SetStyle(style *TextStyle, start int32, end int32) {
	this.CheckLayout()
	var pool *cocoa.NSAutoreleasePool = nil
	if !cocoa.NSThreadIsMainThread() {
		pool = castcocoaNSObjectTococoaNSAutoreleasePool(cocoa.NewNSAutoreleasePool().Alloc().Init())
